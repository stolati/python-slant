...
int main(int argc, char **argv)
{
    ...
    if(argc != 3){
        printf("<20x20dh> <seed>, got %i argsn", argc-1);
        return 1;
    }

    char * str_params = argv[1];
    char * seed = argv[2];

    char * aux = NULL;

    game_params *p = default_params();
    decode_params(p, str_params);

    random_state* rand_state = random_new(seed, strlen(seed));

    char * game_desc = new_game_desc(p, rand_state, &aux, 0);
